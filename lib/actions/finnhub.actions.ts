...
import { POPULAR_STOCK_SYMBOLS } from '@/app/_shared/constants';
...
async function fetchJSON<T>(
  ...
  if (!res.ok) {
    ...
    throw new Error(`Fetch failed ${String(res.status)}: ${text}`);
  }
...
export async function getNews(
  ...
  try {
    ...
    for (const art of general) {
      ...
      const key = `${String(art.id)}-${art.url ?? ''}-${art.headline ?? ''}`;
